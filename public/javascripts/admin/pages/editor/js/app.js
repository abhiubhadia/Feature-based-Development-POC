...
function removeElm() {
    $(".htmlpage").delegate(".remove", "click", function (e) {
        ...
        if (confirm("Do you want to delete the selection ?")) {
            e.preventDefault();
            $(this).parent().remove();
...
function downloadLayoutSrc() {
    ...
    finalDoc = $("#download-layout").clone();

    // var t = $("#download-layout").children();
    finalDoc.children('.container').each(function (i) {
        var container = $(this);
    ...
    cleanCustomComponent(finalDoc);

    var finalModel = $($(".htmlpage").clone());

    cleanCustomComponent(finalModel);

    $('textarea#model').val(finalModel.html());
    $('textarea#src').val(style_html(finalDoc.html()));
    $('#download').modal('show');

}

function cleanCustomComponent(doc) {
    doc.find(".customComponent").attr("class", "customComponent");
    doc.find(".customComponent").empty();
}
...
$('#srcSave').click(function () {
    ...
    var page = {
        name: $("#pageName").val(),
        path: $("#pageContextPath").val(),
        source: $("#src").val(),
        editorModel: $("#model").val(),
        enabled: true
    };
    if (pageId && pageId.length > 0) {
        ...
        $.ajax({
            ...
            error: function (result) {
                ...
                $(".alert").text("Page is not updated due to errors");
            }
        });
    } else {
        $.ajax({
